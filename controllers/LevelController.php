...
class LevelController extends Controller
{
    ...
    public function actionSearch(){
        ...
        $query=Level::find()->where(['like','name',$q]);
        $pages=new Pagination(['totalCount'=>$query->count(),'pageSize'=>3,'forcePageParam'=>false,'pageSizeParam'=>false]);
        $task=$query->offset($pages->offset)->limit($pages->limit)->all();
        return $this->render('search',compact('task','pages','q'));
     }
     public function actionAbc()
     {
         $letter = Yii::$app->request->get('letter');
         $this->setMeta('Задачи начинающиеся на букву:' . $letter);
         if (!$letter) return $this->render('abc');
         $query = Task::find()->where(['like', 'name', $letter, '%', false]);
         $pages = new Pagination([
             'totalCount' => $query->count(),
             'pagesSize' => 1,
             'forcePageaParam' => false,
             'pageSizeParam' => false
         ]);
         $task = $query->orderBy('id DESC')
             ->offset($pages->offset)
             ->limit($pages->limit)
             ->all();
         return $this->render('abc', compact('task', 'pages', 'letter'));
     }
  }
